...
class controller_add extends core_controller {
	...
	function add_item() {
		...
		if ($lot_no != NULL && $brand != NULL && $name != NULL && $name_generic != NULL && $unit_price != NULL && $unit_qty != NULL && $qty != NULL && $date_purchase != NULL && $date_expire != NULL) {
			if (strlen($unit_price) > 16 || strlen($qty) > 16 || strlen($date_purchase) > 16 || strlen($date_expire) > 16) {
				...
				if ($this->create->item_add($data)) {
					$_SESSION["alert"] = "Successfully added Item.";
				} else {
